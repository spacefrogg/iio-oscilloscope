...
#define GPIO_CTRL_FIRST_CH		"voltage0"
#define VOLTAGE_MONITOR_1_FIRST_CH	"voltage9"
#define VOLTAGE_MONITOR_2_FIRST_CH	"voltage0"
...
static void cn0540_get_channels()
{
	...
	label = strdup(GPIO_CTRL_FIRST_CH);
	while(1) {
	...
	if(iio_voltage_mon != NULL) {
		if(!strcmp(iio_device_get_name(iio_voltage_mon),
			   VOLTAGE_MONITOR_1)) {
			label = strdup(VOLTAGE_MONITOR_1_FIRST_CH);
		} else {
			label = strdup(VOLTAGE_MONITOR_2_FIRST_CH);
		}
		for(idx = 0; idx < NUM_ANALOG_PINS; idx++) {
